...
class Body2COLMAP_RenderSplat:
    ...
    def render(self, splat_scene, width, height,
        ...
        if path_config is not None:
            ...
            # Reuse cameras from b2c_data, anchor index included
            anchor_frame_index = b2c_data.get("anchor_frame_index")
        ...
        # anchor_frame_index is rebuilt too: our path has its own anchor (or
        # none), so inheriting the input dataset's index would be stale.
        _REBUILT_KEYS = {"cameras", "image_names", "points_3d", "resolution",
                         "focal_length_mm", "splat_path", "anchor_frame_index"}
        if b2c_data:
